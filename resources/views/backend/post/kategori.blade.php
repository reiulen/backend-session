          ...
          <div class="section-body">
            ...
            <div class="row">
              <div class="col-12">
                <div class="card">
                  <div class="card-body">
                    <button class="btn btn-dark mb-3"  data-toggle="modal" data-target="#modalTambah"><i class="fa fa-edit"></i> Tambah Kategori</button>
                      ...
                      <table class="table table-striped" id="table-petugas">
                        <thead>
                          <tr>
                            ...
                            <th class="text-center">Jumlah Post Sesuai Kategori</th>
                            <th></th>
                          </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                <td>{{ $no++ }}</td>
                                <td><a>{{ $row->kategori }}</a></td>
                                <td class="text-center"><div class="badge {{ ($post->where('kategori_id', $row->id)->count() == 0) ? 'badge-primary' : 'badge-danger' }}">{{ ($post->where('kategori_id', $row->id)->count() == 0) ? 'Bisa dihapus' : 'Tidak bisa dihapus' }}</div></td>
                                <td class="text-center">{{ $post->where('kategori_id', $row->id)->count() }}</td>
                                <td>
                                    <div class="dropdown">
                                      <button class="btn btn-none" type="button" id="kategoridrop{{ $row->id }}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fas fa-ellipsis-v"></i></button>
                                        <div class="dropdown-menu" aria-labelledby="kategoridrop{{ $row->id }}">
                                            <a href="#" class="dropdown-item" data-toggle="modal" data-target="#modalEdit{{ $row->id }}"><i class="fa fa-edit"></i> Edit Kategori</a>
                                            @if($post->where('kategori_id', $row->id)->count()==0)
                    ...
                    <div class="float-left col-6 pt-4">
                        <span class="text-small text-dark">*Jika ingin menghapus kategori, hapus Post yang terkait dengan kategori tersebut! atau bisa juga mengganti kategori Postnya!</span>
                    </div>
                    <div class="float-right">
                      {{ $kategori->links() }}
                    </div>
        ...
        <script>
          $(".hapusKategori").click(function(e){
              ...
              $.confirm({
                  ...
                  content: 'Apakah yakin akan tetap mengahapus? Sebelum kategori dihapus, Post dalam kategori ini harus di hapus!',
                  type: 'red',
